...
class cBizPaymentService(Component):
    ...
    @restapi.method(
        [(['/hawb/batch/'], "POST")],
        input_param=restapi.CerberusValidator("_validator_create_batch_v2"),
        )
    def cargo_v2_batch_payments(self, **kwargs):
        if "company_id" not in kwargs:
            kwargs["company_id"] = 1
        create_payment = self._create_batch_payment_v2(kwargs)
        return create_payment

    def _create_batch_payment_v2(self, values):
        hawb = values['name']
        payment_date = values['payment_date']
        company_id = values['company_id']
        invoices = self.env['account.move'].search([('ref','like',hawb),('state','=','posted'),('move_type','in',['out_refund','out_invoice'])])
        ...
        invoices_data = self.env['cbiz.api.cargoapi'].balance_check(invoices)
        payment_total = 0
        for payment in values["payment"]:
            payment_total += payment["amount"]
        if (invoices_data['total_balance'] > 0 and payment_total < 0) or (invoices_data['total_balance'] < 0 and payment_total > 0):
            raise ValidationError("Wrong payment amount, balance is: " + str(invoices_data['total_balance']))
        else:
            if abs(invoices_data['total_balance']) == 0:
                raise ValidationError(hawb + ": No Balance")
            if (abs(invoices_data['total_balance']) - abs(payment_total)) < 0:
                raise ValidationError(hawb + ": Overpayment")
        payment_return = []
        for value in values["payment"]:
            invoices = self.env['account.move'].search([('ref','like',hawb),('state','=','posted'),('move_type','in',['out_refund','out_invoice'])])
            invoices_data = self.env['cbiz.api.cargoapi'].balance_check(invoices)
            payment_type = value['payment_type']
            payment_amount = value['amount']
            payment_amount_balance = value['amount']
    ...
    def _validator_create_batch_v2(self):
        schema = {
            "payment_type":{"type": "string", "required": True, "allowed": ["cash","bank","stcpay"]},
            "amount":{"type": "float", "required": True}
            }
